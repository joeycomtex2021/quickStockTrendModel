...
class Portfolio(object):
    def __init__(self,initialvalue,maxexposure,stockMaxAllocation):
        ...
        self.shortpositionvalue=0
        self.interval_date_id_dict=None
        

    def setIntervalIDDateDict(self,idDateDict):
        self.interval_date_id_dict=idDateDict   # must be a Dictionary object with key of "date" and value of "interger"
...
#            print (positionvalue)
    ...
    def __openStockPosition(self,symbol,opendate,openrule,shares,entryprice):        
        ...
        if (not positionAdded):
            # failed to add stock position
            print (('  __addPosition() failed! symbol:%s opendate:%s shares:%d entryprice:%f') % (symbol,opendate,shares, entryprice))
            return False

        #self.stockOpenPositions[key][3].
        stockPositionRecord=self.stockOpenPositions[symbol]
        if (stockPositionRecord is None):
            print (('  __openStockPosition() Failed to get position record! symbol:%s') % (symbol))
            return False
        
        # use the position price to update cash value
        newEntryPrice=entryprice
        positionObject = stockPositionRecord[3]
        if (positionObject is not None):
            if (self.interval_date_id_dict is not None):
                nIntervalID=self.interval_date_id_dict[opendate]
                newEntryPrice=positionObject.getSymbolPrice(nIntervalID)    
                if (newEntryPrice!=entryprice):
                    newEntryPrice = newEntryPrice 
                #cvcp_window_intervals_dict[interval_id];
        #adjust the position price
        stockPositionRecord[2]=newEntryPrice  #position record: ['opendate','shares','entryprice', positionObject]
        #getSymbolPrice(self,interval_id)
        #step 2:  add trade
        if (positionAdded and not (shares is None)):
            if (shares != 0):
                #step3:  update cash value
                self.cashvalue -= float(shares)*newEntryPrice   #adjust cash value
                #step 4:  add trades
                self.stocktrades = self.stocktrades.append({'opendate':opendate,'symbol':symbol,'entryprice':newEntryPrice,'shares':shares,'openrule':openrule,'closedate':'2020-1-1','closeprice':0,'closerule':0},ignore_index=True)
            if (shares>0):            
                #step 5:  reporting
    ...
    def __closeStockPosition(self,symbol,opendate,closedate,closeprice,closerule):
        if (self.__existsPositionWithSymbolOpenDate(symbol,opendate)):
            ...
            stockPositionRecord=self.stockOpenPositions[symbol]
            if (stockPositionRecord is None):
                print (('  __closeStockPosition() Failed to get position record! symbol:%s') % (symbol))
                return False
            
            shares=stockPositionRecord[1]
            if (shares==0):
                print (('  __closeStockPosition() failed since Position is 0 sized! symbol:%s') % (symbol))
                return False
            
            # use the position price to update cash value
            newClosePrice=closeprice
            positionObject = stockPositionRecord[3]
            if (positionObject is not None):
                if (self.interval_date_id_dict is not None):
                    nIntervalID=self.interval_date_id_dict[closedate]
                    newClosePrice=positionObject.getSymbolPrice(nIntervalID)    
                    if (newClosePrice != closeprice):
            ...
            self.stocktrades.loc[(self.stocktrades['symbol']==symbol)&(self.stocktrades['opendate']==opendate),'closeprice']=newClosePrice
            
            #remove the position
            self.__removePosition(symbol)
            
            #update cash value
            self.cashvalue += float(shares)*newClosePrice
    ...
    def buystockWithPctAllocation(self,symbol,opendate,entryprice,selldate,openrule,pctallocation):
        dollarallocation = self.portfoliovalue * pctallocation
        return self.buystockWithAllocation(symbol,opendate,entryprice,selldate,openrule,dollarallocation)
